        ...
        if (WorldGen.CORAL_EXTRA) {
            for (int i = -rand.nextInt(16) - rand.nextInt(4); i < rand.nextInt(16) + rand.nextInt(4); i++) {
                ...
                for (int j = -rand.nextInt(12) - rand.nextInt(i != 3 ? 1 : 5); j < rand.nextInt(14) + rand.nextInt(1); j++) {
                    int zPos = z + j;
                    if (coralCanReplace(world.getBlock(xPos, y - 1, zPos)) &&
                        BlockHelper.isWater(world, xPos, y + 1, zPos) &&
                        BlockHelper.isWater(world, xPos, y + 2, zPos)) {
            ...
            for (int j1 = z - l; j1 <= z + l; ++j1) {
                ...
                if (k1 * k1 + l1 * l1 <= l * l) {
                    for (int i2 = y; i2 <= y; ++i2) {
                        if (coralCanReplace(world.getBlock(i1, i2 - 1, j1)) &&
                            BlockHelper.isWater(world, i1, i2 + 1, j1) &&
                            BlockHelper.isWater(world, i1, i2 + 2, j1)) {
